...
contract NFTMarketplace is Initializable, ReentrancyGuard {
    ...
    event NFTListedForSale(uint indexed id, address indexed seller, uint price);
    event NFTListedForAuction(uint indexed id, address indexed seller, uint startingPrice);
    event NFTSold(uint indexed id, address indexed buyer);
    event NFTBid(uint indexed id, address indexed bidder, uint amount);
    ...
    mapping(uint => Item) public idToItemForSale;
    mapping(uint => Item) public idToItemForAuction;

    function startNFTSale(NFTType _type, address _contractAddress, uint _price, uint _tokenId, uint _quantity) public {
        ...
        emit NFTListedForSale(idForSale, msg.sender, _price);
    }

    function startNFTAuction(NFTType _type, address _contractAddress, uint _price, uint _tokenId, uint _quantity) public {
        ...
        emit NFTListedForAuction(idForAuction, msg.sender, _price);
    }
    ...
    function buyNFT(uint _id) public payable nonReentrant {
        ...
        uint price = msg.value.mul(95).div(100);
        ...
        if (item.nftType == NFTType.ERC721) {
            ...
            NFT.safeTransferFrom(address(this), msg.sender, item.tokenId, item.quantity, "");
        }
    ...
    function isNFTApprovedForMarketplace(NFTType _type, address _contractAddress, address _ownerAddress) public view returns (bool) {
        if (_type == NFTType.ERC721) {
            IERC721 NFT = IERC721(_contractAddress);
    ...
    function bid(uint Id) payable public {
        ...
        require(msg.value > item.price, "Wrong Price!"); // This line seems redundant, consider removing
        ...
        emit NFTBid(Id, msg.sender, msg.value);
    }
    ...
    function finishNFTAuction(uint Id) payable public {
        ...
        uint price = item.price.mul(95).div(100);
        payable(item.seller).transfer(price);
        payable(owner).transfer(msg.value.sub(price));
